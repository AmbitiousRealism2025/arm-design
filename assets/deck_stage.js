 ...
 *
 * Features:
 * - Fixed-size canvas (default 1920x1080), auto-scale, and letterboxing
 * - Keyboard navigation (Left/Right/Space/Home/End/Esc)
 * - Left/right click-zone navigation
 * - Slide counter
 * - localStorage persistence for the current slide
 * - Speaker notes postMessage support
 * - Hash navigation (#slide-5 jumps to slide 5)
 * - Print-to-PDF support (Cmd+P / Ctrl+P, one slide per page)
 * - Automatic data-screen-label values for each slide
 *
 * Usage:
 *   <deck-stage>
...
(function() {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Render Shadow DOM first. It is independent from child parsing timing.
      this._render();

      // Defensive setup: if the script is in <head> rather than after
      // </deck-stage>, the parser may not have processed child sections yet.
      // Delay initialization so child nodes are available.
      const init = () => {
        this._collectSlides();
      ...
      if (this.ownerDocument.readyState === 'loading') {
        // Document is still parsing; wait until sections are available.
        this.ownerDocument.addEventListener('DOMContentLoaded', init, { once: true });
      } else {
        // Document is parsed; collect on the next frame.
