...
(async () => {
  ...
  if (hasReady) {
    // Second safety layer: actively reset animation time to zero. This helps
    // when an HTML file does not strictly follow the starter tick template,
    // such as when lastTick uses performance.now() and font-loading time gets
    // counted into the first frame delta. See animation-pitfalls.md section 12.
    ...
    if (seekCorrected) {
      // Wait two rAFs so seek applies and the t=0 frame renders.
